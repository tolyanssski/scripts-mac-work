...
## Базовые принципы работы со структурами и файлами

ВАЖНО: ОДНА СТРУКТУРА ДОЛЖНА ЛЕЖАТЬ СТРОГО В ОДНОМ ОТДЕЛЬНОМ ФАЙЛЕ! Исключения - всякие мелкие вспомогательные локальные DTO. Но если структура представляет собой конкретную реализацию чего-либо и у этой структуры есть публичные методы - правило жесткое, такая структура со всеми своими методами должна лежать в своем собственном файле.

ЕЩЕ ОДНО ПРАВИЛО ПО СТРУКТУРАМ: если я явно не указал про это в задаче или если задача не касается внесения дополнений в структуры/пакеты где такое правило не соблюдено, ЗАПРЕЩАЕТСЯ объединять структуры по общему признаку в общие базовые встраиваемые структуры.

Не бойся того что строгое следование этим двум правилам выше в итоге приведет к дублированию кода, это "кажущееся" дублирование,
основная цель задачи не сделать минимум кода, а сделать максимум поддерживаемости
этого кода в будущем. *ГЛАВНЫЙ ПРИНЦИП: РАЗДЕЛЯЙ И ВЛАСТВУЙ!*
...
И должен быть отдельно сервис (структура), выполняющий роль фасада:
в нем содержатся все сервисы из пула, а в публичных методах фасада определяется,
...
- Логируй все действия и ошибки, но многие ошибки уведомлений не должны прерывать поток.
...
ВАЖНО: по умолчанию во все сервисы-структуры пробрасывай логгер зависимостью,
и во всех выполняемых методах в коде по умолчанию, помимо прочего, ОБЯЗАТЕЛЬНО логируй 
записи вида "сейчас начинаю такое-то действие" перед вызовом какого-то другого метода (приватного или публичного, своего или чужого),
а после успешного завершения вызова метода логируй запись типа "такое-то действие успешно завершено".
В случае ошибки вызова метода, перед ее возвратом логируй запись "такое-то действие
завершилось с ошибкой: <текст ошибки>".

Во всех записях лога префиксом в квадратных скобках указывай название сервиса + название
метода в коде которого эта запись произошла, а также если в окружающем коде
ставится Redis Lock на какой-либо конкретный идектификатор (например user_id, transaction_id и тд) - обязательно во всех записях лога указывай этот идентификатор, чтобы по нему можно было искать.

Записи лога делай по умолчанию Info (никаких debug и прочих), а если ошибка то Error.
