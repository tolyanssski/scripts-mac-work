...
перед началом работы с сущностью (апдейтом и тд) ставится лок путем создания специальной записи в БД или Redis или еще каком-либо хранилище.
В нашем продукте во всех сервисах используется лок через Redis. В проектах
...
- ключ каждого лока должен быть строкой в формате: `<project_name>:<package_name>:<structure_name>:<method_name>:<ID>`, где ID - это либо ID блокируемой сущности (если блокируется сущность), либо самое значение блокируемого идентификатора (если блокируется что-то другое);
...
После установки лока в методе-обработчике делай заново SELECT данной сущности из репозитория по ее ID, чтобы гарантировать что обработка сущности будет идти со свежими данными, актуальными на момент успешного лока.
